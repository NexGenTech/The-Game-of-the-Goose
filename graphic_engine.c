...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game){
  ...
  for(int i = 0; i < n_objects ; i++){
    Id object = object_get_id(game->objects[i]);
    if ((obj_loc = game_get_object_location(game,object)) != NO_ID){
      sprintf(dummie1, "  %s:%ld,",object_get_name(game->objects[i]), obj_loc);
  ...
  if ((player_obj = player_get_object(game->player)) != NO_ID){
  ...
  if ((last_die = die_get_last_roll(game->die)) != -1){
